...
void process_token(TokenStream* tokenStream, Teletext* teletext, State* currentState, State* nextState, int row, int column) {
  ...
  token = tokenStream->tokens[row][column];
	...
	if(token->type == SYMBOL || token->type == UNUSED) {
		handle_symbol_token(teletext, token, &(teletext->pixels[row][column]), currentState, nextState, row, column);
	} else {
		handle_control_token(tokenStream, &(teletext->pixels[row][column]), currentState, nextState, row, column);
	}
}
